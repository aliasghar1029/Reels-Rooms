...
## Restricted PC / Mobile Login (optional)
If you need to use the site on a computer where Google sign-in and downloads
are blocked, there's an optional backend that lets you log in with a plain
username + password instead. See `backend/BACKEND_SETUP_GUIDE.md`.
...
- `backend/` — optional backend for restricted-PC / ID+password login